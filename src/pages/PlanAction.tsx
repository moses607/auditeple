    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">Réf. : Actions correctives découlant de la cartographie des risques — Recommandation + date limite</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card><CardContent className="pt-4 text-center"><p className="text-2xl font-bold">{actions.filter(a => a.statut === 'À lancer').length}</p><p className="text-xs text-muted-foreground">À lancer</p></CardContent></Card>
        <Card><CardContent className="pt-4 text-center"><p className="text-2xl font-bold">{actions.filter(a => a.statut === 'En cours').length}</p><p className="text-xs text-muted-foreground">En cours</p></CardContent></Card>
        <Card><CardContent className="pt-4 text-center"><p className="text-2xl font-bold text-green-600">{actions.filter(a => a.statut === 'Réalisé').length}</p><p className="text-xs text-muted-foreground">Réalisées</p></CardContent></Card>
        <Card><CardContent className="pt-4 text-center"><p className="text-2xl font-bold">{actions.length}</p><p className="text-xs text-muted-foreground">Total</p></CardContent></Card>
      </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-muted-foreground">{a.processus}</span>
                    {overdue && <Badge variant="destructive" className="text-[10px]">EN RETARD</Badge>}
                  </div>
                  <p className="text-sm text-muted-foreground">{a.risqueLabel}</p>
                  ...
                  {a.observations && <p className="text-xs italic text-muted-foreground mt-1">{a.observations}</p>}
                </div>
