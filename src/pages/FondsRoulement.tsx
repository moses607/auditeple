...
export default function FondsRoulementPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold">Analyse du fonds de roulement (FDRm)</h1>
      <p className="text-xs text-muted-foreground">Réf. : Rapport IGAENR 2016-071 — Outil FDRm Académie d'Aix-Marseille — Demande de prélèvement sur fonds de roulement</p>
      ...
      {A > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <Card><CardContent className="pt-4 text-center"><p className="text-lg font-bold">{fmt(A)}</p><p className="text-xs text-muted-foreground">FDR comptable</p></CardContent></Card>
          <Card><CardContent className="pt-4 text-center"><p className={`text-lg font-bold ${B >= 0 ? 'text-green-600' : 'text-destructive'}`}>{fmt(B)}</p><p className="text-xs text-muted-foreground">FDR disponible</p></CardContent></Card>
          <Card><CardContent className="pt-4 text-center"><p className={`text-lg font-bold ${joursDisponibles < 30 ? 'text-destructive' : joursDisponibles < 60 ? 'text-orange-500' : 'text-green-600'}`}>{joursDisponibles}j</p><p className="text-xs text-muted-foreground">Jours disponibles</p></CardContent></Card>
          <Card><CardContent className="pt-4 text-center"><p className="text-lg font-bold">{fmt(seuilCritique)}</p><p className="text-xs text-muted-foreground">Seuil critique (30j)</p></CardContent></Card>
          <Card><CardContent className="pt-4 text-center"><p className={`text-lg font-bold ${margePrelevement > 0 ? 'text-green-600' : 'text-destructive'}`}>{fmt(margePrelevement)}</p><p className="text-xs text-muted-foreground">Marge de prélèvement</p></CardContent></Card>
        </div>
      )}
