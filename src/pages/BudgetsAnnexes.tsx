      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground">Budgets Annexes — M9.6</h1>
      ...
      <Tabs defaultValue="gestion" className="space-y-4">
        ...
        <TabsContent value="gestion" className="space-y-4">
          ...
          {filtered.map(x => {
            ...
            return (
              <Card key={x.id} className="transition-shadow hover:shadow-md">
          ...
          {!selectedBA ? (
            ...
            <>
              ...
              {baAuditItems.map((item, idx) => {
                ...
                if (!def) return null;
                return (
                  <Card key={item.id} className={`transition-shadow hover:shadow-md ${item.conforme === 'non' ? 'border-l-4 border-l-red-400' : item.conforme === 'oui' ? 'border-l-4 border-l-emerald-400' : item.conforme === 'partiel' ? 'border-l-4 border-l-amber-400' : 'border-l-4 border-l-muted'}`}>
